...
class graph_AccX(pg.PlotWidget):
    def __init__(self):
        super().__init__()
        self.setTitle('Acc X', color='w')
        self.setLabel('left', 'Acc X ( m/s²)')
        self.setLabel('bottom', 'Time(s)')

        #graph style
        pen = pg.mkPen(color=(241,8,86), width=3)
    ...
    def update(self, value):
        x = self.x[-1]
        self.x[:-1] = self.x[1:]
        self.x[-1] = x +0.010 #Since update graph every 0.2sec
