...
// 모터를 시계 방향으로 지정된 바퀴 수만큼 회전시키는 함수
void rotateMotor(int revolutions) {
  digitalWrite(MOTOR_PIN1, HIGH);
  digitalWrite(MOTOR_PIN2, LOW);
  analogWrite(MOTOR_ENA, 200); // 속도를 조절할 수 있는 PWM 값 (0-255)

  long rotationDelay = 200 * revolutions; // 각 회전에 200ms 씩 할당
  delay(rotationDelay);
