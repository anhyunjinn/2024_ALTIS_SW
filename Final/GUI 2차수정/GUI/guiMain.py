...
class guiMain(QMainWindow):
    ...
    def __init__(self):
        ...
        self.elapsed_timer1 = QElapsedTimer()
        self.elapsed_timer2 = QElapsedTimer()
        self.interval1 = 21  # 목표 간격 (밀리초) for drawGraphs
        self.interval2 = 21  # 목표 간격 (밀리초) for drawGraphic
        self.elapsed_timer1.start()
        self.elapsed_timer2.start()
        self.rendering_thread = threading.Thread(target=self.render_loop)
        self.rendering_thread.start()

        self.timer = QTimer(self)
        self.timer.timeout.connect(self.check_update)
        self.timer.start(1)  # 1ms 간격으로 체크
    ...
    def render_loop(self):
        while True:
            # 데이터 처리 및 렌더링 로직
            self.render_signal.update_signal.emit()
            threading.Event().wait(1/21)  # 약 30fps로 설정

    def paintGL(self):
        gl.glClear(gl.GL_COLOR_BUFFER_BIT)
        
    def check_update(self):
        if self.elapsed_timer1.elapsed() >= self.interval1:
            self.drawGraphs()
            self.elapsed_timer1.restart()
        
        if self.elapsed_timer2.elapsed() >= self.interval2:
            self.drawGraphic()
            self.elapsed_timer2.restart()
