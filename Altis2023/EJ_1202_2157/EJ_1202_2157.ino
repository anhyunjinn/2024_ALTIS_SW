...
void setup() {
  ...
  pinMode(17,OUTPUT);
  ejServo.attach(servoPin);
  ejServo.write(90);  //ì´ê¸° ê°ë
}
...
void ej() {
  /*Timer*/
  if (sensor.alt > 50 && launchTime == 0) {
    launchTime = millis();
  }

  if (launchTime > 0 && millis() - launchTime >= 8000) {
    sensor.ej = true;
  }
  ...
  if (sensor.alt > 120 && sensor.yaw *sensor.yaw + sensor.pitch * sensor.pitch > 6400) {
    sensor.ej = true;
  }
...
void CF() {
  ...
  if (prevTime != 0) {
    interval = (millis() - prevTime) * 0.001;  //ì´ì  ìê°ê³¼ì ì°¨ì´
  }
  prevTime = millis();  //íì¬ ìê°ì ì´ì  ìê°ì¼ë¡ ì ì¥
